...
for e in range(episode_num):
    ...
    if C[-1] == min_C[-1]:
        for i, s in enumerate(S_list):
            Q[s[0]][s[1]][s[2]][s[3]][s[4]][s[5]][O_list[i]] += 1/C[-1]
        best_start_list = start_list
        best_O_list = O_list
        best_C_J = Ft.C_J
    if e == episode_num - 1:
        plt.figure(1)
        print("最佳工件顺序列表:", best_O_list)  # 工件顺序列表
        print("最佳调度各工序完工时间:", best_C_J)  # 各工序完工时间
        print("最佳开始时间列表:", best_start_list)
        node.draw_gantt(best_start_list, best_O_list, best_C_J)
